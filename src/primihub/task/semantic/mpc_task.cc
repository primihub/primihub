  ...
  if (function_name == "logistic_regression") {
    ...
    algorithm_ = std::dynamic_pointer_cast<AlgorithmBase>(
        std::make_shared<primihub::LogisticRegressionExecutor>(
            config, dataset_service, this->getTaskContext().getLinkContext()));
    ...
    try {
      algorithm_ = std::dynamic_pointer_cast<AlgorithmBase>(
          std::make_shared<primihub::cryptflow2::MaxPoolExecutor>(
              ...
              this->getTaskContext().getLinkContext()));
    ...
    std::shared_ptr<primihub::MPCSendRecvExecutor> executor =
        std::make_shared<primihub::MPCSendRecvExecutor>(
            config, dataset_service, this->getTaskContext().getLinkContext());
    ...
    try {
      ...
      if (accuracy == "D32")
        algorithm_ = std::dynamic_pointer_cast<AlgorithmBase>(
            std::make_shared<primihub::ArithmeticExecutor<D32>>(
                config, dataset_service, link_context));
      else
        algorithm_ = std::dynamic_pointer_cast<AlgorithmBase>(
            std::make_shared<primihub::ArithmeticExecutor<D16>>(
                config, dataset_service, link_context));
    } catch (const std::runtime_error &error) {
      LOG(ERROR) << error.what();
      algorithm_ = nullptr;
    ...
    try {
      algorithm_ = std::dynamic_pointer_cast<AlgorithmBase>(
          std::make_shared<primihub::MissingProcess>(
              config, dataset_service,
              this->getTaskContext().getLinkContext()));
    } catch (const std::runtime_error &error) {
      LOG(ERROR) << error.what();
