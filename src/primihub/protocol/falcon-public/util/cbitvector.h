...
#define GetMask(len) (((1 << (len)) - 1))
			...
			template <class T>
			void Set(T val, int pos, int len) { SetBits((BYTE *)&val, pos, len); }
			...
			template <class T>
			void XOR(T val, int pos, int len) { XORBits((BYTE *)&val, pos, len); }
			 ...
			 */
			template <class T>
			T Get(int i) { return Get<T>(i * m_nElementLength, m_nElementLength); }
			template <class T>
			void Set(int i, T val) { return Set<T>(val, i * m_nElementLength, m_nElementLength); }
			/*
			 * The same as the above methods only for two-dimensional access
			 */
			template <class T>
			T Get2D(int i, int j) { return Get<T>((i * m_nNumElementsDimB + j) * m_nElementLength, m_nElementLength); }
			template <class T>
			void Set2D(int i, int j, T val) { return Set<T>(val, (i * m_nNumElementsDimB + j) * m_nElementLength, m_nElementLength); }
