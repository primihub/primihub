...
/* Fill the bitvector with random values and pre-initialize the key to the
 * seed-key*/
...
void CBitVector::Create(int numelements, int elementlength, BYTE *seed,
                        int &cnt) {
...
void CBitVector::Create(int numelementsDimA, int numelementsDimB,
                        int elementlength) {
...
void CBitVector::Create(int numelementsDimA, int numelementsDimB,
                        int elementlength, BYTE *seed, int &cnt) {
...
void CBitVector::SetBits(BYTE *p, int pos, int len) {
  ...
  for (i = 0; i < len / (sizeof(BYTE) * 8); i++, posctr++) {
    temp = p[i];
    m_pBits[posctr] = (m_pBits[posctr] & RESET_BIT_POSITIONS[lowermask]) |
                      ((temp << lowermask) & 0xFF);
    m_pBits[posctr + 1] =
        (m_pBits[posctr + 1] & RESET_BIT_POSITIONS_INV[uppermask]) |
        (temp >> uppermask);
  ...
  if (remlen) {
    ...
    if (remlen <= uppermask) {
      // cout << "Setting " << remlen  << " lower bits with lowermask = " <<
      // lowermask << ", and temp " << (unsigned int) temp << endl;
      m_pBits[posctr] =
          (m_pBits[posctr] & (~(((1 << remlen) - 1) << lowermask))) |
          ((temp << lowermask) & 0xFF);
    } else {
      // cout << "Setting " << remlen  << " bits with lowermask = " << lowermask
      // << ", uppermask = " << uppermask << ", and temp = " << (unsigned int)
      // temp << endl;
      m_pBits[posctr] = (m_pBits[posctr] & RESET_BIT_POSITIONS[lowermask]) |
                        ((temp << lowermask) & 0xFF);
      m_pBits[posctr + 1] =
          (m_pBits[posctr + 1] & (~(((1 << (remlen - uppermask)) - 1)))) |
          (temp >> uppermask);
    }
  }
}
...
void CBitVector::GetBits(BYTE *p, int pos, int len) {
  ...
  for (i = 0; i < len / (sizeof(BYTE) * 8); i++, posctr++) {
    p[i] =
        ((m_pBits[posctr] & GET_BIT_POSITIONS[lowermask]) >> lowermask) & 0xFF;
    p[i] |= (m_pBits[posctr + 1] & GET_BIT_POSITIONS_INV[uppermask])
            << uppermask;
  }
  int remlen = len & 0x07;
  if (remlen) {
    ...
    if (remlen <= uppermask) {
      // cout << "Getting " << remlen  << " lower bits with lowermask = " <<
      // lowermask << ", " << (unsigned int) m_pBits[posctr] << endl;
      p[i] = ((m_pBits[posctr] & (((1 << remlen) - 1 << lowermask))) >>
              lowermask) &
             0xFF;
      // cout << "p[i] = " << (unsigned int) p[i] << endl;
    } else {
      // cout << "Getting upper" << endl;
      // cout << "Getting " << remlen  << " upper bits with lowermask = " <<
      // lowermask << ", " << (unsigned int) m_pBits[posctr] << ", and uppermask
      // = "
      //		<< uppermask << ", " << (unsigned int) m_pBits[posctr+1]
      //<< endl;
      p[i] = ((m_pBits[posctr] & GET_BIT_POSITIONS[lowermask]) >> lowermask) &
             0xFF;
      p[i] |= (m_pBits[posctr + 1] & (((1 << (remlen - uppermask)) - 1)))
              << uppermask;
      // cout << "p[i] = " << (unsigned int) p[i] << endl;
    }
  }
...
// XOR bits given an offset on the bits for p which is not necessarily divisible
// by 8
...
void CBitVector::ORByte(int pos, BYTE p) { m_pBits[pos] |= p; }
...
void CBitVector::GetBytes(BYTE *p, int pos, int len) {
  ...
  BYTE *dst = p;
  // Do many operations on REGSIZE types first and then (if necessary) use
  // bytewise operations
  GetBytes((REGSIZE *)dst, (REGSIZE *)src,
           ((REGSIZE *)dst) + (len >> SHIFTVAL));
...
void CBitVector::XORBytes(BYTE *p, int pos, int len) {
  ...
  // Do many operations on REGSIZE types first and then (if necessary) use
  // bytewise operations
  XORBytes((REGSIZE *)dst, (REGSIZE *)src,
           ((REGSIZE *)dst) + (len >> SHIFTVAL));
  dst += ((len >> SHIFTVAL) << SHIFTVAL);
  src += ((len >> SHIFTVAL) << SHIFTVAL);
...
void CBitVector::SetBytes(BYTE *p, int pos, int len) {
  ...
  // Do many operations on REGSIZE types first and then (if necessary) use
  // bytewise operations
  SetBytes((REGSIZE *)dst, (REGSIZE *)src,
           ((REGSIZE *)dst) + (len >> SHIFTVAL));
  dst += ((len >> SHIFTVAL) << SHIFTVAL);
  src += ((len >> SHIFTVAL) << SHIFTVAL);
...
void CBitVector::ANDBytes(BYTE *p, int pos, int len) {
  ...
  // Do many operations on REGSIZE types first and then (if necessary) use
  // bytewise operations
  ANDBytes((REGSIZE *)dst, (REGSIZE *)src,
           ((REGSIZE *)dst) + (len >> SHIFTVAL));
  dst += ((len >> SHIFTVAL) << SHIFTVAL);
  src += ((len >> SHIFTVAL) << SHIFTVAL);
...
unsigned int CBitVector::GetInt(int bitPos, int bitLen) {
  ...
  if (bitLen == 1)
    return ret;
  // cout << "MpBits: " << (unsigned int) m_pBits[i-1] << ", ret: " << ret <<
  // endl;
  j = 8 - j;
  for (k = bitLen - j; i < (bitPos + bitLen + 7) / 8 - 1; i++, j += 8, k -= 8) {
    // ret |= REVERSE_BYTE_ORDER[m_pBits[i+pos]] << (i*8);
    ret |= m_pBits[i] << j;
    // cout << "MpBits: " <<(unsigned int) m_pBits[i] << ", ret: " << ret << ",
    // j: " << j << endl; m_pBits[i+pos] = (
    // ((REVERSE_NIBBLE_ORDER[(p>>((2*i)*4))&0xF] << 4) |
    // REVERSE_NIBBLE_ORDER[(p>>((2*i+1)*4))&0xF])  &
    // 0xFF);//((p>>((2*i+1)*4))&0xF) | ((p>>((2*i)*4))&0xF) & 0xFF;
  }
  ret |= (m_pBits[i] & SELECT_BIT_POSITIONS[k])
         << j; // for the last execution 0<=k<=8
  // cout << "MpBits: " <<(unsigned int) m_pBits[i] << ", ret: " << ret << ", k
  // = " << k <<  ", selects: " << (unsigned int) SELECT_BIT_POSITIONS[k] << ",
  // j: " << j << endl;
  return ret;
}
...
void CBitVector::EklundhBitTranspose(int rows, int columns) {
  ...
  for (int i = 0, j; i < LOG2_REGISTER_SIZE; i++, srcidx *= 2) {
    ...
    // cout << "numrounds = " << numrounds << " iterations: " <<
    // LOG2_REGISTER_SIZE << ", offset = " << offset << ", srcidx = " << srcidx
    // << ", destidx = " << destidx << endl; Preset the masks that are required
    // for bit-level swapping operations
    ...
    // If swapping is performed on byte-level reverse operations due to
  ...
  for (int i = LOG2_REGISTER_SIZE, j, swapoffset = 1, dswapoffset; i < numiters;
       i++, srcidx *= 2, swapoffset = swapoffset << 1) {
    destidx = offset * srcidx;
    ...
    for (int j = 0; j < rows / (srcidx * 2); j++) {
      for (p = 0, lim = rowbptr + destidx; p < destidx && rowbptr < lim;
           p++, rowaptr++, rowbptr++) {
