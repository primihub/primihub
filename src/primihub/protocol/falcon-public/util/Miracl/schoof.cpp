...
#define RAISE '#'
    ...
    int main(int argc, char **argv)
    {
        ...
        forever
        {
            ...
            if (pbits <= 100)
                d = pow((Big)2, 48);
            if (pbits > 100 && pbits <= 120)
                d = pow((Big)2, 56);
            if (pbits > 120 && pbits <= 140)
                d = pow((Big)2, 64);
            if (pbits > 140 && pbits <= 200)
                d = pow((Big)2, 72);
            if (pbits > 200)
                d = pow((Big)2, 80);
            ...
            for (i = 1; i < nl; i++)
            {
                ...
                { // prime power
                    for (jj = 0; jj < start_prime; jj++)
                        if (lp % (int)l[jj] == 0)
                        {
                            ...
                            {
                                permisso[tau] = FALSE;
                                if (tau % (int)l[jj] == (int)t[jj])
                                    permisso[tau] = TRUE;
                                if ((lp - tau) % (int)l[jj] == (int)t[jj])
                                    permisso[tau] = TRUE;
                            }
                            break;
                        }
                ...
                for (tau = 1; tau <= lp / 2; tau++)
                {
                    ...
                    { // we have a result. Now compare Y's
                        ...
                        {
                            if (tau == 1)
                                Ry = ZT3YP * (Pf[tau + 2] * P2f[tau - 1] + P2f[tau + 1]);
                            else
                                Ry = ZT3YP * (Pf[tau + 2] * P2f[tau - 1] - Pf[tau - 2] * P2f[tau + 1]);
