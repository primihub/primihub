...
#define HERD 5
    ...
    Big kangaroo(Big p, Big order, Big ordermod)
    {
        ...
        forever
        {
            ...
            for (i = 0; i < HERD; i++)
                start[i] = middle + 13 * ordermod * i; // tame ones
            for (i = 0; i < HERD; i++)
                start[HERD + i] = 13 * ordermod * i; // wild ones
    ...
    int main(int argc, char **argv)
    {
        ...
        forever
        {
            ...
            sl[3] = 8;
            sl[4] = 9;
            sl[5] = 0; // do low prime powers
            SCHP = 9;

            if (pbits <= 256)
                d = pow((Big)2, 64);
            else
                d = pow((Big)2, 72); // kangaroos do more work
            ...
            if (!escape)
                for (i = 1; accum <= d; i++)
                {
                        ...
                        for (jj = 1; jj < r; jj++)
                        {
                            ...
                            else if (jac(tau, lp) == 1)
                            {
                                ...
                                if (candidates == phi(r))
                                {
                                    ...
                                    cout << (p + 1 - tau) % lp << "," << (p + 1 + tau) % lp << "," << flush;
                            }
                        }
                        if (escape)
                            break;
                    ...
                    for (lambda = 1; lambda <= (lp - 1) / 2; lambda++)
                    {
                        ...
                        {
                            if (lambda == 1)
                                Ry = (Pf[lambda + 2] * P2f[lambda - 1] + P2f[lambda + 1]) / 4;
                            else
                                Ry = (Pf[lambda + 2] * P2f[lambda - 1] - Pf[lambda - 2] * P2f[lambda + 1]) / 4;
                            Ty = YP * P3f[lambda];
                        }
