...
void AES_ecb_encrypt_chunk_in_out(block *in, block *out, unsigned nblks,
                                  AES_KEY_TED *aesKey) {
...
void AES_ctr_hash_gate(block *in, block *out, int gate, int numOfParties,
                       AES_KEY_TED *aesKey) {
#ifdef ENABLE_SSE
...
void AES_ctr_hash_gate(block *in, block *out, int gate, int numOfParties,
                       __m128i seedIn1, __m128i seedIn2) {
  __m128i counter, temp = {0};
  __m128i *seed =
      static_cast<__m128i *>(_aligned_malloc(2 * sizeof(__m128i), 16));
  ;
  seed[0] = seedIn1;
  seed[1] = seedIn2;
  AES_KEY_TED aes_key;
...
__m128i *pseudoRandomFunction(__m128i seed1, __m128i seed2, int gate,
                              int numOfParties) {
  __m128i counter = {0}, temp; // = { 0 };
  __m128i *result = static_cast<__m128i *>(
      _aligned_malloc(numOfParties * sizeof(__m128i), 16));
  seed[0] = seed1;
  seed[1] = seed2;
  AES_KEY_TED aes_key;
  AES_set_encrypt_key((unsigned char *)seed, 256, &aes_key);
...
void pseudoRandomFunctionNew(__m128i seed1, __m128i seed2, int gate,
                             int numOfParties, __m128i *out) {
  __m128i counter = {0}, temp; // = { 0 };
...
void AES_init(int numOfParties) {
  // temp = static_cast<__m128i *>(_aligned_malloc(numOfParties *
  // sizeof(__m128i), 16));
  switch (numOfParties) {
  case 3:
...
void AES_free() { _aligned_free(temp); }
...
bool pseudoRandomFunctionwPipelining(__m128i seed1, __m128i seed2, int gate,
                                     int numOfParties, __m128i *out) {
  __m128i counter = {0};
  ...
  AES_KEY_TED aes_key;
  AES_set_encrypt_key((unsigned char *)seed, 256, &aes_key);
  __m128i *temp_in = static_cast<__m128i *>(
      _aligned_malloc(sizeof(__m128i) * (numOfParties + 1), 16));
  __m128i *temp_out = static_cast<__m128i *>(
      _aligned_malloc(sizeof(__m128i) * (numOfParties + 1), 16));
  ...
  if (numOfParties == 4)
    AES_ecb_encrypt_for_5(temp_in, temp_out, numOfParties + 1, &aes_key);
  else
    AES_ecb_encrypt_chunk_in_out(temp_in, temp_out, numOfParties + 1, &aes_key);
  memcpy(out, temp_out,
         numOfParties * sizeof(__m128i)); //-----------------------Check if
                                          // order of arguments is correct
...
bool fixedKeyPseudoRandomFunctionwPipelining(__m128i seed1, __m128i seed2,
                                             int gate, int numOfParties,
                                             __m128i *out) {
  __m128i counter = {0};
  ...
  for (int i = 0; i < numOfParties + 1; i++) {
#ifdef ENABLE_SSE
    temp_in[i] = _mm_xor_si128(
        _mm_xor_si128(_mm_set_epi32(0, 0, gate, i), dseed1), ddseed2);
#else
    TODO("Implement it.");
#endif
  }
  if (numOfParties == 4)
    AES_ecb_encrypt_for_5(temp_in, temp_out, numOfParties + 1, &fixed_aes_key);
  else
    AES_ecb_encrypt_chunk_in_out(temp_in, temp_out, numOfParties + 1,
                                 &fixed_aes_key);
...
// assumes nblks==7
void AES_ecb_encrypt_for_7(block *in, block *out, int nblks,
                           AES_KEY_TED *aesKey) {
  unsigned j, rnds = ROUNDS(aesKey);
  const block *sched = ((block *)(aesKey->rd_key));
  // block temp[4];
...
// assumes nblks==5
void AES_ecb_encrypt_for_5(block *in, block *out, int nblks,
                           AES_KEY_TED *aesKey) {
  unsigned j, rnds = ROUNDS(aesKey);
  const block *sched = ((block *)(aesKey->rd_key));
  // block temp[4];
...
// assumes nblks==3
void AES_ecb_encrypt_for_3(block *in, block *out, int nblks,
                           AES_KEY_TED *aesKey) {
  unsigned j, rnds = ROUNDS(aesKey);
  const block *sched = ((block *)(aesKey->rd_key));
  // block temp[4];
...
void AES_ecb_encrypt_for_4(block *in, block *out, int nblks,
                           AES_KEY_TED *aesKey) {
  unsigned j, rnds = ROUNDS(aesKey);
  const block *sched = ((block *)(aesKey->rd_key));
  // block temp[4];
