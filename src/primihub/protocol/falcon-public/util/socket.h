...
public:
  BOOL Socket() {
    ...
    success =
        (m_hSock = socket(PF_INET, SOCK_STREAM, IPPROTO_TCP)) != INVALID_SOCKET;
    ...
    // setsockopt(m_hSock, IPPROTO_TCP, TCP_MAXSEG , (char*)&OptSegSizeLen,
    // OptSegSize);
    //  disable tcp slow start - (false)
    // setsockopt(m_hSock, IPPROTO_TCP, TM_TCP_SLOW_START , (char*)&bOptVal,
    // bOptLen); cout << "TCP_MAXSEG: " << TCP_MAXSEG << endl;
    return success;
  }
  ...
  void AttachFrom(CSocket &s) { m_hSock = s.m_hSock; }

  void Detach() { m_hSock = INVALID_SOCKET; }
...
public:
  ...
  BOOL Bind(USHORT nPort = 0, string ip = "") {
    ...
    if (ip != "") {
      int on = 1;
      setsockopt(m_hSock, SOL_SOCKET, SO_REUSEADDR, (const char *)&on,
                 sizeof(on));
    ...
    /*if (bind(m_hSock, (sockaddr *)&sockAddr, sizeof(sockaddr_in)) ==
    SOCKET_ERROR)
    {
  ...
  BOOL Listen(int nQLen = 5) { return listen(m_hSock, nQLen) >= 0; }
  ...
  BOOL Connect(string ip, USHORT port, LONG lTOSMilisec = -1) {
    ...
    if (lTOSMilisec > 0) {
      setsockopt(m_hSock, SOL_SOCKET, SO_RCVTIMEO, (char *)&lTOSMilisec,
                 sizeof(lTOSMilisec));
    }
