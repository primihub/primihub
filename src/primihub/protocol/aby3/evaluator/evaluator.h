...
class Sh3Evaluator {
 public:
  ...
  Sh3Task asyncConstMul_test(Sh3Task dependency, const i64& A,
                             si64 B, si64Matrix& C);

  Sh3Task asyncMul(Sh3Task dependency, const si64& A, const si64& B,
			             si64& C, u64 shift);

  template<Decimal D>
  Sh3Task asyncMul(Sh3Task dependency, const sf64<D>& A,
  ...
  Sh3Task asyncMul(Sh3Task dependency, const sf64Matrix<D>& A,
    ...
    return asyncMul(dependency, A.i64Cast(), B.i64Cast(),
                    C.i64Cast(), D + shift);
  }
  ...
  Sh3Task asyncMul(Sh3Task dep, const si64Matrix& A, 
  ...
  Sh3Task asyncConstFixedMul(Sh3Task dependency, const f64<D>& A,
                             const sf64<D>& B, sf64<D>& C) {
    return asyncConstFixedMul(dependency, A.mValue, B.i64Cast(),
                              C.i64Cast(), D);
  }
